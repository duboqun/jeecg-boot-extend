...
public class ActZprocessServiceImpl extends ServiceImpl<ActZprocessMapper, ActZprocess> implements IActZprocessService {
    ...
    public void sendActMessage(LoginUser fromUser, LoginUser toUser, ActBusiness act, String taskName, Boolean sendMessage, Boolean sendSms, Boolean sendEmail) {
        String title = "您有一个新的审批任务：" + act.getTitle();
...
//        msgMap.put("remark", "请进入待办栏，尽快处理！");
        ...
        String msgText = "您有一个新的审批任务：" + act.getTitle();
...
//            DySmsHelper.sendSms(toUser.getPhone(), obj, DySmsEnum.REGISTER_TEMPLATE_CODE)
    ...
    public ProcessNodeVo getNextNode(String procDefId, String currActId, String procInsId) {
        ...
        log.info("currActId: {}", currActId);
        log.info("procInsId: {}", procInsId);
        log.info("actBusiness: {}", actBusiness);
        ...
        for (ActivityImpl activityImpl : activitiList) {
            if (activityImpl.getId().equals(currActId)) {
                ...
                log.info("pvmActivity: {}", pvmActivity.toString());
                ...
                if ("userTask".equals(type)) {
                    ...
                    // 多实例信息
                    if(pvmActivity.getProperty("multiInstance") != null && StringUtils.isNotEmpty(pvmActivity.getProperty("multiInstance").toString())) {
                        ActivityImpl activityImpl1 = (ActivityImpl)pvmActivity;
                        if(activityImpl1.getActivityBehavior() instanceof MultiInstanceActivityBehavior){
                            MultiInstanceActivityBehavior multiInstanceActivityBehavior = (MultiInstanceActivityBehavior)activityImpl1.getActivityBehavior();
                            node.setCollectionExpressionParam(multiInstanceActivityBehavior.getCollectionExpression().toString().substring(2,multiInstanceActivityBehavior.getCollectionExpression().toString().length() -1));
                        }
                    }
                    // 设置关联用户
                    log.info("pvmActivity.getId() : {}", pvmActivity.getId());
                    log.info("actBusiness.getTableName() : {}", actBusiness.getTableName());
                    log.info("actBusiness.getTableId() : {}", actBusiness.getTableId());
