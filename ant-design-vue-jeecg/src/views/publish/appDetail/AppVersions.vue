...
<script>
  import AppVersionEdit from "@/views/publish/appDetail/AppVersionEdit";
  ...
  export default {
    ...
    computed: {
      ...
      url() {
        ...
        return shortCode ? (document.location.protocol + "//" + window.location.host + "/preview/" + shortCode) : "";
      },
    },
    mounted() {
    ...
    methods: {
      getAppList() {
        ...
        this.getAction(`/api/apps/${this.id}`, {}).then(res => {
          if (res.success) {
      ...
      download(filename) {
        window.open(document.location.protocol + "//" + window.location.host + "/jeecg-boot/api/" + "appVersions/downloadApk/" + filename);
      },
      gotoPreview() {
        window.open(document.location.protocol + "//" + window.location.host + "/preview/" + this.appInfo.shortCode, "_blank");
      },
      handleSubmit(tabActive) {
        this.$refs.form.validate(valid => {
          if (valid) {
            ...
            putAction("/api/apps",body).then(res=>{
              if(res.success){
