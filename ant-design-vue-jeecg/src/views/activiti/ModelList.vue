  ...
  <a-card :bordered="false">
    ...
    <a-table :scroll="{x:1280,y:innerHeight/2}" bordered
      ...
      <span slot="make" slot-scope="text, record">
        ...
        <a href="javascript:void(0);" @click="updatelc(record.id)">设计</a>
        <a-divider type="vertical" />
        <a-popconfirm
          title="是否确认删除?"
...
<script>
  ...
  import { deleteAction, getAction,downFile,downloadFile } from '@/api/manage'
  export default {
    name: "ModelList",
    ...
    methods: {
      ...
      /*导出xml模型文件*/
      exportModel(id, type) {
        let url = 'http://localhost:8080/jeecg-boot/activiti/models/activiti/export/' + id + '/' + type;
      ...
      exportdiagram(record) {
        // let url = 'http://localhost:8080/jeecg-boot/activiti/models/activiti/exportDiagram' + '?modelId=' + id;
        // let a = document.createElement("a");
        // a.setAttribute("href", url);
        // document.body.appendChild(a);
        // a.click();
