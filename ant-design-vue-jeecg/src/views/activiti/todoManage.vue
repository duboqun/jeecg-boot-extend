...
export default {
  ...
  methods: {
      ...
      this.getAction(this.url.getNextNode,{procDefId:v.procDefId, currActId:v.key, procInstId:v.procInstId}).then(res => {
        ...
        if (res.success) {
          ...
          if (res.result.users && res.result.users.length > 0) {
            ...
            this.form.collectionExpressionParam = res.result.collectionExpressionParam
            console.log(this.form.collectionExpressionParam)
          } else {
            this.form.assignees = [];
            this.showAssign = false;
          }
    ...
    changeBackTask(v) {
      ...
      this.userLoading = true;
      this.getAction(this.url.getNode,{nodeId: v,tableId: this.form.tableId, tableName: this.form.tableName}).then(res => {
        this.userLoading = false;
        if (res.success) {
          if (res.result.users && res.result.users.length > 0) {
            this.assigneeList = res.result.users;
    ...
    backTask(v) {
      ...
      this.form.tableName = v.tableName;
      this.showAssign = false;
      this.modalTaskVisible = true;
